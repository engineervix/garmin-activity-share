...
@task(
    help={
        "base": "run pip-compile on requirements/base.in",
        "dev": "run pip-compile on requirements/dev.in",
        "prod": "run pip-compile on requirements/production.in",
    }
)
def pip_compile(c, base=False, dev=False, prod=False):
    """run pip-compile to generate requirements"""
    if base:
        c.run(
...
def lint(c, fix=False):
    ...
    if fix:
        ...
        c.run("flake8 tests garmin_activity_share", pty=True)
